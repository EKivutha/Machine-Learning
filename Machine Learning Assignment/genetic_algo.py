...
#individuals with an upper and lower limit are created and genes randomly created
...
#determine fitness value of an individual.    
def fitness_calculation(individual):
    fitness_value = sum(individual)
    return fitness_value    

#take the fitness values of the population and output the next generation individuals.
def selection(generation, method='Fittest Half'):
    ...
    if method == 'Roulette Wheel': #equal chance for selecting each individual,Fitter individuals have a higher chance to be selected.
        selected = []
    ...
    elif method == 'Fittest Half':# fittest half of the candidate solutions are selected to move to the next generation.
    ...
    elif method == 'Random': #individuals selected randomly.
...
def pairing(elit, selected, method = 'Fittest'):
    ...
    if method == 'Fittest': #Fitter individuals are paired together
        parents = [[individuals[x],individuals[x+1]] 
                   for x in range(len(individuals)//2)]
    if method == 'Random': #random pairing
        parents = []
        for x in range(len(individuals)//2):
            parents.append(
    ...
    if method == 'Weighted Random': #pairing is random but fitter individuals have a higher chance to be selected for pairing
...
def mating(parents, method='Single Point'):
    if method == 'Single Point': #after a single point genes are replaced with those of the other parent to create two offsprings
    ...
    if method == 'Two Pionts': #genes between two points are replaced with the genes of the other parent to create two offsprings.
...
def mutation(individual, upper_limit, lower_limit, muatation_rate=2, 
    ...
    if method == 'Gauss': #mutating gene is replaced with a number that is generated according to gauss distribution around the original gene
        for x in range(muatation_rate):
            mutated_individual[x] = \
            round(individual[x]+gauss(0, standard_deviation), 1)
    if method == 'Reset': #the original gene is replaced with a randomly generated gene.
        for x in range(muatation_rate):
...
def next_generation(gen, upper_limit, lower_limit):
    elit = {} #elite individuals that must be passed to the next generation
    next_gen = {}
